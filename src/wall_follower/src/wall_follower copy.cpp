...

{
	...
	// Left Corner Turning			
	else if (scan_data_close_[LEFT_FRONT] > 0.5 || scan_data_close_[LEFT] > 0.9) {
		std::cout << "LEFT_FRONT: " << scan_data_close_[LEFT_FRONT] << std::endl;
		target_ang = 1.8, target_lin = 0.0;
	}

	// CORRECTIVE LEFT AND RIGHTS

	else if (scan_data_close_[FRONT_LEFT] < 0.3 && scan_data_close_[FRONT_LEFT] > 0) {
		std::cout << "CORRECTIVE RIGHT: " << scan_data_close_[FRONT_LEFT] << std::endl;
		target_ang = -1.2, target_lin = 0.02;
	}

	else if (scan_data_close_[FRONT_RIGHT] < 0.3 && scan_data_close_[FRONT_RIGHT] > 0) {
