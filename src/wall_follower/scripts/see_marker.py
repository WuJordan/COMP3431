...
# Need to adjust - manually calculate
...
class SeeMarker(Node):
	...
	def listener_callback(self, data):
		...
		# Or I could do it this way -> directly to CV2
		current_frame = self.br.compressed_imgmsg_to_cv2(data, 'bgra8')

 
		# Convert ROS Image message to OpenCV image

		# PERCHANCE NEED TO GET RID OF THIS
		...
		# Find pink blob -> Every marker has a pink section (Hence IF there is a Marker: try and find which marker it is.) 
		...
		if pink_blob:
			...
			for c in ["blue", "green", "yellow"]:
				...
				if blob:
					...
					# If distance between centres is greater than tolerance, leave.
					# Might need to adjust tolerance.
...
#						print(f'pink_x = {pink_x}, pink_y = {pink_y}, h = {pink_h}')
					...
					"""
					# Checks if the pink marker is up or down on the cylinder.
...
def segment(current_frame, hsv_frame, colour):
	...
	# BLOBS MIGHT BE INCORRECT -> NUMLABELS = 1
	blobs = cv2.connectedComponentsWithStats(mask, 4, cv2.CV_32S)

	# Display masked image
	# cv2.imshow("result", result)
	cv2.imshow(f"result_{colour}", result)  # unique window name
...
#		print(colour, x, y, w, h, area, cx, cy)
		...
		if area > largest:
			...
			# Ratio of height and width should be known
			# Aspect ratio

			# Need to change the value of 0.8
			aspect_ratio = h/w
			if aspect_ratio < 0.8:
				if cx < centre:
					cx += h-w
